...
const AdminLogin = () => {
	...
	const valueCheck = (user, password) => {
		...
		const passwordRegex =
			/^[A-Za-z0-9,.\-;:?!()%"@$/€ñÑáÁéÉíÍóÓöÖőŐúÚüÜűŰ\s]+$/;
	...
	return (
		<TransitionParent isFlex>
			<form
				...
				className="grid grid-cols-4 w-[40rem] bg-primary rounded-xl shadow-xl items-center p-4 gap-8"
			>
				<h1 className="col-span-2 col-start-2 text-center text-text text-[2rem] font-[600]">
					Admin Login
				</h1>
				<span className="col-span-2 col-start-2 flex flex-col gap-y-4">
				...
				<button className={`${blogNewFormStyle.button} col-span-2 col-start-2`}>
					Login
				</button>
			</form>
		</TransitionParent>
